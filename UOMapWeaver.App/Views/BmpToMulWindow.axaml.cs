...
public sealed partial class BmpToMulView : UserControl, IAppStateView
{
    ...
    private async void OnConvertTerrainTo24Bit(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        if (!await ViewHelpers.ConfirmOverwriteAsync(this,"Overwrite 24-bit BMP?", outputPath))
        {
    ...
    private async void OnGenerate(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (!await ViewHelpers.ConfirmOverwriteAsync(this,"Overwrite output files?", outputFiles))
            {
                StatusText.Text = "Conversion cancelled.";
                StatusTextPreview.Text = StatusText.Text;
                AppStatus.SetWarning(StatusText.Text);
