...
public sealed class Bmp8StreamReader : IDisposable
{
    ...
    public Bmp8StreamReader(string path)
    {
        ...
        _stream.Seek(FileHeaderSize + DibHeaderSize, SeekOrigin.Begin);
        Palette = ReadPalette(_stream);
    }
    ...
    public void ReadRow(int y, Span<byte> destination)
    {
        if (_disposed)
        {
            throw new ObjectDisposedException(nameof(Bmp8StreamReader));
        }
        ...
        _rowBuffer.AsSpan(0, _width).CopyTo(destination);
    }
    ...
    private static BmpPaletteEntry[] ReadPalette(Stream stream)
    {
        var palette = new BmpPaletteEntry[PaletteEntries];
        Span<byte> buffer = stackalloc byte[PaletteEntries * 4];
