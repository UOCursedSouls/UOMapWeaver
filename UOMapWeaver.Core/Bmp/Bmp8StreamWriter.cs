...
public sealed class Bmp8StreamWriter : IDisposable
{
    ...
    public Bmp8StreamWriter(string path, int width, int height, BmpPaletteEntry[] palette)
    {
        if (width <= 0 || height <= 0)
        {
    ...
    public void WriteRow(ReadOnlySpan<byte> row)
    {
        if (_disposed)
        {
            throw new ObjectDisposedException(nameof(Bmp8StreamWriter));
        }
